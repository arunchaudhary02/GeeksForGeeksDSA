...
class Solution {
    static Node deleteAllOccurOfX(Node head, int x) {
        ...
        while (temp != null) {
            // If the current node matches the value to delete
            if (temp.data == x) {
                ...
                // If the current node is in the middle or at the end
                else {
                    Node lastNode = temp.prev;
                    Node nextNode = temp.next;

                    lastNode.next = nextNode; // Update the previous node's next
                    if (nextNode != null) {
                        nextNode.prev = lastNode; // Update the next node's prev if not null
                    }
                }
                ...
                nodeToDelete.prev = null; // Clear references for garbage collection
            } else {
